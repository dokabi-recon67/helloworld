...
// Define HW_MAX_SERVERS if not already defined (to avoid circular dependency)
#ifndef HW_MAX_SERVERS
#define HW_MAX_SERVERS 16
#endif

// Forward declaration only - actual typedef is in helloworld.h
// Don't typedef here to avoid redefinition errors
struct hw_ctx;
...
int hw_shutdown_request_drain(struct hw_ctx* ctx);
int hw_shutdown_flush_queues(struct hw_ctx* ctx);
int hw_shutdown_close_sockets(struct hw_ctx* ctx);
