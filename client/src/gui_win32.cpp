        ...
        case WM_TIMER:
            if (wp == ID_TIMER_UPDATE) {
                ...
                if (g_ctx && g_ctx->status == HW_CONNECTED) {
                    ...
                    // Active stealth maintenance (state-aware)
                    // Rotate TLS fingerprint intelligently (state-aware, not during bursts)
                    static time_t last_fingerprint_rotation = 0;
                    time_t now = time(NULL);
                    time_t elapsed = now - last_fingerprint_rotation;
                    
                    // State-aware rotation: only rotate during idle periods
                    // Maintain stability during active sessions (15-30 min)
                    // Allow rotation during idle (10-20 min)
                    // Avoid rotation during burst edges
                    int rotation_interval = (elapsed < 600) ? 1800 : 900;  // Active: 30min, Idle: 15min
                    if (last_fingerprint_rotation == 0 || (elapsed > rotation_interval && (rand() % 3) != 0)) {
                        hw_rotate_tls_fingerprint();
                        last_fingerprint_rotation = now;
                    }
                    
                    // Generate active dummy traffic
                    hw_generate_active_dummy_traffic(g_ctx);
                }
