        ...
        case WM_TIMER:
            if (wp == ID_TIMER_UPDATE) {
                ...
                if (g_ctx && g_ctx->status == HW_CONNECTED) {
                    ...
                    // Active stealth maintenance (10/10 stealth)
                    // Rotate TLS fingerprint periodically (every 5-15 minutes)
                    static time_t last_fingerprint_rotation = 0;
                    time_t now = time(NULL);
                    if (last_fingerprint_rotation == 0 || (now - last_fingerprint_rotation) > 300) {
