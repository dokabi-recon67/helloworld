...
int hw_generate_stealth_stunnel_config(hw_ctx_t* ctx, const char* config_path) {
    ...
    // Use 'all' for maximum compatibility, then disable old protocols via options
    fprintf(f, "sslVersion = all\n");  // Support all TLS versions, then restrict via options
    fprintf(f, "options = NO_SSLv2\n");
    fprintf(f, "options = NO_SSLv3\n");
    // Note: TLSv1.2 and TLSv1.3 will be preferred via cipher selection
    // NO_TLSv1 and NO_TLSv1_1 options may not be supported in all stunnel versions
    ...
    // Note: compression option not supported in stunnel client mode
