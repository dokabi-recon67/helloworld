...
static int g_fingerprint_index = -1;
...
// Generate stunnel config with browser-like TLS settings
int hw_generate_stealth_stunnel_config(hw_ctx_t* ctx, const char* config_path) {
    ...
    fprintf(f, "; HelloWorld Stealth Configuration\n");
    ...
    fprintf(f, "sslVersion = TLSv1.2\n");  // Start with TLS 1.2, upgrade to 1.3
    fprintf(f, "options = NO_SSLv2\n");
    fprintf(f, "options = NO_SSLv3\n");
    fprintf(f, "options = NO_TLSv1\n");
    fprintf(f, "options = NO_TLSv1_1\n");
    
    // Use browser-like cipher suites (stunnel will negotiate best available)
    fprintf(f, "ciphers = HIGH:!aNULL:!eNULL:!EXPORT:!DES:!RC4:!MD5:!PSK:!SRP:!CAMELLIA\n");
    
    // Randomize connection behavior
    fprintf(f, "TIMEOUTclose = 0\n");
    fprintf(f, "TIMEOUTidle = 300\n");  // 5 minute idle timeout (like browsers)
...
// Advanced ML evasion - break ML classification patterns
void hw_evade_ml_classification(char* data, size_t* len) {
    ...
    // 1. Vary packet sizes more aggressively
    size_t size_variation = (*len * 10) / 100;  // ±10% variation
    *len += (rand() % (size_variation * 2)) - size_variation;
    
    // 2. Add entropy variations (not perfectly uniform)
    for (size_t i = 0; i < *len && i < 64; i += 8) {
        // Every 8 bytes, slightly modify to break uniformity
        data[i] ^= (char)((i / 8) % 256);
    }
    
    // 3. Add micro-patterns that look like HTTP headers
    if (*len > 100 && *len < 2000) {
        // Small packets might be HTTP requests - add header-like structure hint
        // (This is subtle, not actual headers)
        if (rand() % 3 == 0) {
            // 33% chance: add subtle structure hint
            data[0] = (data[0] & 0xF0) | 0x08;  // Subtle pattern
        }
    }
}
