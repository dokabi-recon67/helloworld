...
// Get random browser fingerprint (with rotation support)
static const browser_fingerprint_t* hw_get_random_fingerprint(void) {
    // Rotate fingerprint periodically to avoid static patterns
    static time_t last_rotation = 0;
    time_t now = time(NULL);
    
    // Rotate every 5-15 minutes to mimic browser behavior
    if (g_fingerprint_index < 0 || (now - last_rotation) > (300 + rand() % 600)) {
        srand((unsigned int)time(NULL) ^ (unsigned int)now);
